...
slurm_args = "--partition={partition} --qos={qos} --mem={mem} --ntasks={ntasks} --ntasks-per-node={ntasks} --ntasks-per-core={ntasks-per-core} --time={time} --hint={hint} --output={output} --error={error} -n {n} -J {J}".format(**job_properties["cluster"])
